...
export default function MintTab() {
    ...
    const {
        userBalance: shezmuBalance,
        getUserBalance: getUserShezmuBalance,
        approve: shezmuApprove,
    } = useERC20Token(shezmuAddress);
    ...
    const shezmuTimeRef = useRef<NodeJS.Timeout | null>(null);
    ...
    const handleInputGuardianAmount = (guardianAmount: string) => {
        ...
        if (regex.test(guardianAmount)) {
            ...
            if (Number(guardianAmount) > guardianInfo?.mintLimit) {
                setInputError(`You can't mint Guardian than ${guardianInfo?.mintLimit} at one time.`);
            } else {
                setInputError("");
            }
        }
    ...
    const handleShezmuApprove = async () => {
        ...
        try {
            setIsApproving((prevState) =>
                prevState.map((value, index) => (index === 0 ? true : value))
            );
            const txnHash = await shezmuApprove(
                guardianAddress,
                parseEther((Number(guardianAmount) * guardianInfo?.pricePerGuardian).toString())
            );
            ...
            setIsApproving((prevState) =>
                prevState.map((value, index) => (index === 0 ? false : value))
            );
        } catch (err) {
            console.log(err);
    ...
    const handleSelectedTokenApprove = async () => {
        ...
        try {
            ...
            setIsApproving((prevState) =>
                prevState.map((value, index) => (index === 1 ? false : value))
            );
        } catch (err) {
            console.log(err);
            setError((err as ErrorTypes).message);
    ...
    const isMintButtonDisabled = selectedToken?.address && (
        !walletConnected || Number(guardianAmount) <= 0 || inputError !== "" ||
        !isTokenApproved || isMinting || !isShezmuApproved || !guardianInfo?.allFeeTokens ||
        guardianInfo?.allFeeTokens.length === 0 || shezmuBalance <= 0 || !shezmuBalance
    );
    ...
    // useEffect functions
    useEffect(() => {
        if (shezmuTimeRef.current) {
            clearTimeout(shezmuTimeRef.current);
        }
        shezmuTimeRef.current = setTimeout(() => {
            if (guardianInfo?.pricePerGuardian && guardianAmount !== undefined && shezmuAllowance !== undefined)
                if (shezmuAllowance > 0)
                    setIsShezmuApproved(!(Number(guardianAmount) > 0) || shezmuAllowance >= Number(guardianAmount) * guardianInfo?.pricePerGuardian)
                else
                    setIsShezmuApproved(false)
        }, 1000)
        return () => {
            if (shezmuTimeRef.current) {
                clearTimeout(shezmuTimeRef.current);
    ...
    }, [shezmuAllowance, guardianAmount, guardianInfo?.pricePerGuardian])
    ...
    }, [walletConnected, updateShezmuAllowance, updateSelectedTokenAllowance, getUserShezmuBalance]);
        ...
        <div className='flex flex-col gap-8'>
            <div className='flex flex-col gap-4'>
                <div className='w-full bg-[#27272A] p-6 rounded-[40px] flex flex-col sm:flex-row items-start sm:items-center'>
                    ...
                    <div className='text-[#F8D044] text-[28px] sm:text-[32px] font-semibold leading-[102%] overflow-hidden'>{shezmuBalance && shezmuBalance.toLocaleString(undefined, { maximumFractionDigits: 2, minimumFractionDigits: 0 })} <span className='font-[Newsreader] font-normal'>Shezmu</span></div>
                </div>
                <div className='w-full bg-[#27272A] p-6 rounded-[40px] flex flex-col lg:flex-row items-start lg:items-center justify-between'>
                    <div className='flex flex-col lg:flex-row items-start lg:items-center'>
                        <div className='text-[#A1A1AA] py-2 lg:p-0'>
                            <div className='leading-[120%] text-xs font-bold w-[130px]'>Guardian quantity</div>
                            <div className='leading-[120%] text-[10px]'>Max {guardianInfo?.mintLimit} per TXN</div>
                        </div>
                        <div>
                            <div className='flex flex-col lg:flex-row items-start lg:items-center gap-2 lg:gap-6'>
                                <input placeholder='0' className={`text-[32px] font-semibold leading-[102%] ${inputError ? 'border border-red-600' : 'border-none'} outline-none rounded-2xl bg-[#18181B] text-[#F8D044] text-center py-1 w-32`} value={guardianAmount} onChange={(e) => handleInputGuardianAmount(e.target.value)} />
                                <div className='text-xs font-light hidden lg:block text-white'>
                                    ...
                                    <div className='leading-[120%]'>Guardian rewards{" "}{(guardianInfo?.rewardPerDay).toLocaleString(undefined, { maximumFractionDigits: 6, minimumFractionDigits: 0, })}{" "}Shezmu per day</div>
                                </div>
                            </div>
                            {inputError !== '' && <div className='text-xs text-red-600'>{inputError}</div>}
                        </div>
                    </div>
                    <div className='text-xs font-light block lg:hidden mt-2 lg:mt-0'>
                        <div className='leading-[120%]'>1 Guardian = {guardianInfo?.pricePerGuardian} Shezmu</div>
                        <div className='leading-[120%]'>Guardian rewards{" "}{(guardianInfo?.rewardPerDay).toLocaleString(undefined, { maximumFractionDigits: 6, minimumFractionDigits: 0 })}{" "}Shezmu per day</div>
                        <div className='leading-[120%] font-bold mt-2 lg:mt-0 text-transparent bg-clip-text bg-gradient-to-b from-[#0084FE] to-[#6FCEEC]'>Total Shezmu : {guardianAmount || 0} *{" "}{guardianInfo?.pricePerGuardian} ={" "}{Number(guardianAmount) * guardianInfo?.pricePerGuardian}</div>
                    </div>
                    <div className='flex items-center jsutify-end gap-6 w-full lg:w-auto'>
                        <div className='leading-[120%] font-bold text-xs hidden lg:block text-end text-transparent bg-clip-text bg-gradient-to-b from-[#0084FE] to-[#6FCEEC]'>Total Shezmu : {guardianAmount || 0} *{" "}{guardianInfo?.pricePerGuardian} ={" "}{Number(guardianAmount) * guardianInfo?.pricePerGuardian}</div>
                        <ApproveBtn isTokenApproved={isShezmuApproved} className={`mt-2 lg:mt-0 w-full`} onClick={handleShezmuApprove} isApproving={isApproving[0]} />
                    </div>
                </div>
                <div className='w-full bg-[#27272A] p-6 rounded-[40px] flex flex-col sm:flex-row items-start sm:items-center justify-between'>
                    ...
                    <div className='flex flex-col md:flex-row items-start sm:items-center gap-1 md:gap-4 w-full sm:w-auto mt-2 sm:mt-0'>
                        ...
                        />
                        <ApproveBtn className={`mt-2 sm:mt-0`} isTokenApproved={isTokenApproved} isApproving={isApproving[1]} onClick={handleSelectedTokenApprove} />
                    </div>
                </div>
            </div>
